...
SECONDS = constants.time.SECONDS
...
def initialise_PMT(exposure = 20):
    ...
    PMT_COUNT.timing.cfg_samp_clk_timing(rate = RATE, source = "/Dev1/CtrInternalOutput", sample_mode = constants.AcquisitionType.CONTINUOUS)
...
def acquire_button_clicked(*a):
    ...
    if not PMT_ACQUIRE_BUTTON.is_check():
        return
    ...
    while PMT_ACQUIRE_BUTTON.is_checked() and (num_box.get_value() < settings["settings/Acquisition Settings/iterations"] or s["settings/Acquisition Settings/iterations"] == 0):

        databox1.clear()
        databox2.clear()

        settings.sent_to_data_header(databox1)
        settings.sent_to_data_header(databox2)

        databox1['t'] = np.linspace(0, settings["settings/simulated_input/points"]*(settings["settings/Acquisitions Settings/exposure"]), settings["settings/simulated_input/points"])
